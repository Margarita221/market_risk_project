...
DROP TRIGGER IF EXISTS trg_portfolio_refresh_portfolio_values ON portfolio;
CREATE TRIGGER trg_portfolio_refresh_portfolio_values
AFTER UPDATE OF base_currency ON portfolio
FOR EACH ROW
EXECUTE FUNCTION riskapp_refresh_portfolio_value_from_portfolio();

DROP TRIGGER IF EXISTS trg_exchange_rate_refresh_portfolio_values ON exchange_rate;
CREATE TRIGGER trg_exchange_rate_refresh_portfolio_values
AFTER INSERT OR UPDATE OR DELETE ON exchange_rate
FOR EACH ROW
EXECUTE FUNCTION riskapp_refresh_portfolios_from_exchange_rate();
