...
GROUP BY p.id, p.user_id, p.name, p.base_currency, p.initial_value, p.current_value, p.created_at, p.updated_at;
...
SELECT
    ...
    (pp.quantity * i.current_price) AS raw_current_value,
    (pp.quantity * pp.average_purchase_price) * COALESCE(riskapp_get_latest_exchange_rate(i.currency, p.base_currency), 0) AS purchase_value_converted,
    (pp.quantity * i.current_price) * COALESCE(riskapp_get_latest_exchange_rate(i.currency, p.base_currency), 0) AS current_value_converted,
    ((pp.quantity * i.current_price) - (pp.quantity * pp.average_purchase_price)) AS profit_loss_raw,
    (
        ((pp.quantity * i.current_price) - (pp.quantity * pp.average_purchase_price))
        * COALESCE(riskapp_get_latest_exchange_rate(i.currency, p.base_currency), 0)
    ) AS profit_loss_converted,
    CASE
